  ...
  const exportarPDF = () => {
    ...
    const rows = logs.map((log: any) => {
      ...
      return [
        ...
        tipo,
        `${log.campo_editado}: ${log.valor_anterior || "—"} → ${log.valor_novo || "—"} (dia ${log.data_registro ? String(log.data_registro).substring(0, 10).split("-")[2]?.padStart(2, "0") || "--" : "--"})`,
      ];
    });
                    ...
                    return (
                      <TableRow key={log.id}>
                        ...
                        <TableCell className="text-sm text-muted-foreground whitespace-nowrap">
                          ...
                          {" (dia "}
                          {log.data_registro ? String(log.data_registro).substring(0, 10).split("-")[2]?.padStart(2, "0") || "--" : "--"}
                          {")"}
                        </TableCell>
                      </TableRow>
                    );
