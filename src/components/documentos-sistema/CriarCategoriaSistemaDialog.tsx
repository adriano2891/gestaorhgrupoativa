...
import { useCreateCategoriaSistema } from "@/hooks/useDocumentosSistema";
...
export const CriarCategoriaSistemaDialog = ({ open, onOpenChange, categorias }: Props) => {
  const createCategoria = useCreateCategoriaSistema();
  ...
  const onSubmit = async (data: FormData) => {
    await createCategoria.mutateAsync({ nome: data.nome, descricao: data.descricao, cor: data.cor, categoria_pai_id: data.categoria_pai_id });
    onOpenChange(false);
    form.reset();
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[450px]">
        <DialogHeader>
          ...
          <DialogDescription>Crie uma categoria para organizar seus documentos.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={createCategoria.isPending}>
                {createCategoria.isPending && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}Criar Categoria
              </Button>
            </DialogFooter>
          </form>
        </Form>
